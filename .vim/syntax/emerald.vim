...
" most Scala keywords
syn keyword scalaKeyword all at builtin codeof elseif export fix function isfixed monitor object primitive refix signal to unfix wait and attached by confirm end external for if islocal move op process restrict syntactictypeof typeobject var when as awaiting checkpoint const enumeration failure forall immutable locate nameof operation record return suchthat typeof view where assert begin class else exit fielf from initially loop new or recovery returnandfail then unavailable visit while
...
" boolean literals + some extra
syn keyword scalaBoolean true false self nil
